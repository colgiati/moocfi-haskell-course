...
import Data.Array
import qualified Data.Text.Internal.Search as Array
import qualified Data.IntMap.Strict as Array
...
longest :: (Ord a) => [[a]] -> [a]
longest xs = helper (sortBy listCompare xs)
    where helper [] = []
          helper (x:xs) = x

listCompare :: (Ord b) => [b] -> [b] -> Ordering
listCompare a [] = GT
listCompare [] b = LT
listCompare (x:xs) (y:ys)
    | length (x:xs) /= length (y:ys) = compare (length (y:ys)) (length (x:xs))
    | x /= y = compare x y
    | otherwise = LT
...
winner scores player1 player2 = if Map.lookup player1 scores >= Map.lookup player2 scores then player1 else player2
...
freqs xs = helper xs Map.empty
    where helper [] map = map
          helper (x:xs) map = case Map.lookup x map of 
                                Nothing -> helper xs (Map.insert x 1 map)
                                Just y -> helper xs (Map.insert x (y + 1) map)
...
transfer from to amount bank
    ...
    | otherwise = case Map.lookup from bank of
        Nothing -> bank
        Just x -> case Map.lookup to bank of
            Nothing -> bank
            Just y -> if x < amount then bank else Map.insert to (y + amount) (Map.insert from (x - amount) bank)
...
maxIndex xs = helper (Data.Array.indices xs) xs
    where helper :: (Ord a, Ix i) => [i] -> Array i a -> i
          helper [i] xs = i
          helper (i:is) xs = if (xs ! i) > xs ! helper is xs then i else helper is xs
