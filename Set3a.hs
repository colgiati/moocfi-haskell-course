...
palindrome s
    ...
    | head s == last s = palindrome (take (length s - 2) (drop 1 s))
    | otherwise = False
...
powers :: Int -> Int -> [Int]
powers x max = takeWhile (<= max) (map (x ^) [0..max])
...
joinToLength n xs = filter (\x -> length x == n) [first ++ second | first <- xs, second <- xs]
...
exec :: [String] -> Integer -> Integer -> [String]
exec [] x y = []
exec ("up" : cs) x y = exec cs x (y + 1)
exec ("down" : cs) x y = exec cs x (y - 1)
exec ("left" : cs) x y = exec cs (x - 1) y
exec ("right" : cs) x y = exec cs (x + 1) y
exec ("printX" : cs) x y = show x : exec cs x y
exec ("printY" : cs) x y = show y : exec cs x y