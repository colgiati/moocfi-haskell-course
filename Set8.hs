...
combine f (Picture x) (Picture y) = Picture (\a-> f (x a)  (y a))
...
union (Shape fx) (Shape fy) = Shape f
  where f a = fx a || fy a

cut :: Shape -> Shape -> Shape
cut (Shape fx) (Shape fy) = Shape f
  where f a = fx a && not (fy a)
...
paint (Picture a) (Shape s) (Picture b) = Picture f
  where f c = if s c then a c else b c
------------------------------------------------------------------------------
...
instance Transform Zoom where
  apply (Zoom x) (Picture f) = Picture (f . zoomCoord x)
...
instance Transform Flip where
  apply FlipX (Picture f) = Picture g
    where g (Coord x y) = f (Coord (-1 * x) y)
  apply FlipY (Picture f) = Picture g
    where g (Coord x y) = f (Coord x (-1 * y))
...
instance Transform Blur where
  apply Blur (Picture f) = Picture g
    where g c = Color (getAverage getRed c) (getAverage getGreen c) (getAverage getBlue c)
          getAverage k (Coord x y) = (k (f (Coord x y)) + k (f (Coord (x + 1) y)) + k (f (Coord x (y + 1))) + k (f (Coord (x - 1) y)) + k (f (Coord x (y - 1)))) `div` 5
